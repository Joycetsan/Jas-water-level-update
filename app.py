...
st.markdown("请在下方上传相应文件并配置执行任务，系统将在远程完成自动化计算与生成。")

# ---------------------------------------------------------
# 1. 任务功能说明栏（放在最前面，无需输入密码即可看到）
# ---------------------------------------------------------
with st.expander("📖 点击查看【任务 1、2、3】具体处理规则说明", expanded=True):
    st.markdown("""
    * **[1] 任务 1：更新今日库存及 E 列公式填充**
      * 根据 `J3` 单元格的 ASIN 自动匹配并更新今天行的 **F列库存** 数据。
      * 自动向下填充更新今日行及前8行的 **E列公式**。
      * 自动对今天行往前 **第14至40行** 的 E列数据进行**历史数值固化**（将公式转为纯数值，防止后续变动影响历史数据）。
    
    ---
    
    * **[2] 任务 2：任务扫描与状态重排（先同步 D 列，再固化/前移/后移）**
      * **D列同步**：针对今日以后的未来行，若有新增的推荐逻辑自动同步填充并标注绿色背景。
      * **历史固化**：扫描过去20天的数据，对达成历史条件的指标列进行数据锁定。
      * **数据挪动**：自动扫描不符合/超出的数据列，分别向下（后移）或向上（前移）自动寻找合适空行进行重新排列。
    
    ---
    
    * **[3] 任务 3：汇总统计（颜色分类统计写入 E3, F3）**
      * 自动扫描今日及以后的所有排程数据。
      * **E3 单元格**：自动累加统计所有带有**背景填充色**的 C/D 列数值之和。
      * **F3 单元格**：自动累加统计所有**非黑色字体且无背景**的 C/D 列数值之和。
    """)
...
with st.sidebar:
    ...
    # 优化后的任务选择框
    task_options_map = {
        "任务1：更新今日库存/E列公式填充（含E列历史固化）": "任务1",
        "任务2：任务扫描（覆盖同步D、历史固化、前移与后移）": "任务2",
        "任务3：汇总统计（背景色/字体颜色分类统计至E3, F3）": "任务3"
    }
